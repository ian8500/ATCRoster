...
test("staff can acknowledge the current published roster version", async ({ page }) => {
  await page.goto("/login");
  await page.getByLabel(/username/i).fill("inv.atco02");
  ...
  await page.goto(`/roster/${publishedMonth}`);
  await page.getByRole("button", { name: "Acknowledge roster" }).click();
  await expect(page.getByText(/Roster acknowledgement recorded/)).toBeVisible();
