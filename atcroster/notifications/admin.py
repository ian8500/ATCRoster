...
def create_sms_administration_blueprint(
    dependencies: SmsAdministrationDependencies,
) -> Blueprint:
    ...
    def admin_sms_audit():
        ...
        rows = (
            dependencies.SmsAudit.query.filter_by(unit_id=unit_id)
            .order_by(
                dependencies.SmsAudit.sent_at.desc(),
                dependencies.SmsAudit.id.desc(),
            )
            .limit(1000)
            .all()
        )
        actor_ids = {row.sent_by_staff_id for row in rows}
        actors = (
            {
                actor.id: actor
        ...
        try:
            registrations = (
                dependencies.SmsSenderRegistration.query.filter_by(
                    unit_id=dependencies.current_unit_id(),
                    provider="clicksend",
                )
                .order_by(
                    dependencies.SmsSenderRegistration.verification_requested_at.desc()
                )
                .all()
            )
    ...
    def confirm_sms_sender(registration_id: int):
        ...
        try:
            expires_at = (
                ...
                if expiry
                else dependencies.utcnow() + timedelta(days=365)
            )
        except ValueError:
        ...
        row.provider_identifier = (
            request.form.get("provider_identifier") or row.number
        ).strip()[:120]
    ...
    def messagemedia_delivery_webhook():
        ...
        message_id = str(
            payload.get("id") or payload.get("message_id") or payload.get("mtId") or ""
        )
        status = str(
            payload.get("status") or payload.get("statusCode") or "submitted"
        ).lower()[:30]
        if message_id:
    ...
    def register_legacy_endpoints(state) -> None:
        state.app.add_url_rule(
            "/admin/sms-audit", "admin_sms_audit", admin_sms_audit, methods=("GET",)
        )
        state.app.add_url_rule(
            "/admin/sms-senders/<int:registration_id>/confirm",
            "confirm_sms_sender",
            confirm_sms_sender,
            methods=("POST",),
        )
